...
Restaurant.prototype.setData = function(data) {
    if (!data.id) { data.id = this.id || uuid.v1(); }
...
Restaurant.update = function(storage, restaurant_id, fields, callback) {
    storage.update(Restaurant.TABLE_NAME, restaurant_id, fields, function(err, data) {
        if (!callback) {
            console.log('WARN: no callback in Restaurant update');
            return;
        }
